...
class _MyRouteScreenState extends State<MyRouteScreen>
  ...
  Widget byList() {
    final _routeProvider = Provider.of<RouteProvider>(context);
    return _routeProvider.isRouteLoading
        ? _routeProvider.availableRoute.length <= 0
            ? RouteShimmers()
            : Center(
                child: Text(
                  "No Routes available",
        ...
        : ListView.builder(
            ...
            itemBuilder: (context, index) {
              return Column(
                children:
                    _routeProvider.availableRoute[index].stores.map((store) {
                  var _index = _routeProvider.availableRoute[index].stores
                      .indexOf(store);
                  _routeProvider.setSelectedRoute(index, _index);
                  return MyRoutesCard(
                    stores: _routeProvider.availableRoute[index].stores[_index],
